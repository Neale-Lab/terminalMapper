...
for my $file (@files) {
  ...
  mkdir("Histograms/$strain") unless (-d "Histograms/$strain");
    ...
    if ($F[$index+6] == 99) {
       ...
       $R2hits{$F2[$index+1]}{$F2[$index+2]}{$F2[$index]}++;
       $Duplicates{$F[2]}{$F[3]}{$F2[3]}{"99"}++;
    }
    if ($F[$index+6] == 163) {
  ...
  my $outfile = $wd."Histograms/".$strain."/Map.R1R2_".$prefix.".txt";
  ...
  for my $OF ($OUT2, $OUT3) { print $OF "Chr\tPos\tWatson\tCrick\n"; }
   ...
   my $outfile4 = $wd."Analysis/".$strain."/Molecules.".$genome."_".$prefix.".txt";
   open my $OUT4, '>', "$outfile4" or die "$!";
   print $OUT4 "Chr\tCoord-A\tCoord-B\tR1W Freq\tR1C Freq\n";
   foreach my $chrn (nsort keys %Duplicates) {
